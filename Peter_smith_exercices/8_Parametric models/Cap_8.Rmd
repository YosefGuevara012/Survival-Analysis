...
swt <- read.csv("swt.csv", header=T, sep =";")
head(swt)
...
```{r}
i <- order
km.model <- survfit(Surv(swt$time,swt$A) ~ swt$B)
plot(km.model)
```
...
reg_exp <- survreg(Surv(swt$time,swt$A)~swt$B, dist = "exponential") 

reg_weibull<<- survreg(Surv(swt$time,swt$A)~swt$B, dist = "weibull") 

reg_lognormal <- survreg(Surv(swt$time,swt$A)~swt$B, dist = "lognormal") 
